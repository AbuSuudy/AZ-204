...
| ----------- | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ | ------------------------------------------ |
| Event Grid  | Push      | Pushing event to service or web hook.<br><br>It can handle down stream service being offline. Since it retries to send event in a 24 hour period and adds message to blob storage dead letter queue if it's set up.<br><br>Message limit of                                                                                                                                                                                                                                                | Event distribution (discrete events) | React to status changes                    |
| Event Hubs  | Pull  Big data pipeline.  it's a large buffer that's capable of receiving large volumes of data with low latency<br><br>Same message can be read by two consumer groups. Whereas service bus a message is owned by one consumer at a time.<br><br>Message limit of 1MB in standard, premium and 2MB for dedicated. <br><br>For the premium plan 1 PU and one event hub (100 partitions) can approximately offer core capacity of ~5-10 MB/s ingress. You can have max of 16 PU in a namespace. ce. ce. | Event streaming (series)             | Telemetry and distributed data streaming   |
| Service Bus | Pull      | High-value enterprise messaging that can't be dropped or accept duplicates.<br><br>Supports message size of up to 100MB. Largest payload would need to use serv                                                                                                                                                                                                                                                                                                                            | Message                              | Order processing and financial transaction |
...
You could also have event grid topic that subscribe to multiple events. Function app that may deal with email when events take place and consumers can subscribe to a service bus topic and process the information. 

![](Images/Pasted%20image%2020260102154328.png)